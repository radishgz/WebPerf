...
class TextTokenizer extends AbstractTokenizer<List<Node>> {
  ...
  public boolean consume(CodeReader codeReader, List<Node> nodeList) {
    ...
    //System.err.println(this.getClass().toString()+"setCode=="+temp);
  ...
  private boolean inScript(List<Node> nodeList) {
    if (!nodeList.isEmpty()) {
      ...
      if (node.getNodeType() == NodeType.TAG) {
        ...
        //modify by xiehq
        if  ( tag.isEndElement() || tag.getCode().endsWith("/>"))
        	isEndElement=true;
        return !isEndElement && "script".equalsIgnoreCase(tag.getNodeName());
      }
    }
    return false;
  }
