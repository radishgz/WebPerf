          ...
          public Method resolveFunction(String prefix, String localName) {
            if (!ArrayUtils.contains(JSTL_FUNCTIONS, localName) && !ArrayUtils.contains(functionsArray, localName)) {
              createViolation(element.getStartLinePosition(),
            		  "Fix this expression: Unknown function \"" + localName + "\".",element);
            }
  ...
  private void validateExpression(TagNode element, Attribute attribute) {
    ...
    try {
      ...
      if (e.getMessage().startsWith("Error")) {
        createViolation(element.getStartLinePosition(), "Fix this expression: " + (e.getMessage() == null ? "" : e.getMessage()),element);
      }
    }
  }
