  ...
  public void endElement(TagNode element) {
    if (!ignoreTag(element) && !nodes.isEmpty()) {
      ...
      if (!previousNode.getNodeName().equals(element.getNodeName())) {
        createViolation(previousNode.getStartLinePosition(), 
        		"The tag '" + previousNode.getNodeName() 
        		+ "' has no corresponding closing tag.",previousNode);
  ...
  public void endDocument() {
    for (TagNode node : nodes) {
      createViolation(node.getStartLinePosition(), 
    		  "This tag has no corresponding closing tag.",node);
    }
  }
