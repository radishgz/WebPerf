...
public class InternationalizationCheck extends AbstractPageCheck {
  ...
  public void characters(TextNode textNode) {
    if (!isUnifiedExpression(textNode.getCode()) && !isPunctuationOrSpace(textNode.getCode())) {
      createViolation(textNode.getStartLinePosition(), 
    		  "Labels should be defined in the resource bundle.",textNode);
    }
  }
          ...
          if (value != null) {
            ...
            if (value.length() > 0 && !isUnifiedExpression(value) && !isPunctuationOrSpace(value)) {
              createViolation(element.getStartLinePosition(), 
            		  "Labels should be defined in the resource bundle.",element);
              return;
            }
          }
        }
