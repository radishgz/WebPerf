...
public class WhiteSpaceAroundCheck extends AbstractPageCheck {
  ...
  private void checkEndWhitespace(Node node, String code, String end) {
    ...
    if (code.endsWith(end) && code.length() > position) {
      ...
      if (!Character.isWhitespace(ch)) {
        createViolation(node.getStartLinePosition(), "A whitespace is missing before the ending tag at column " + (node.getEndColumnPosition() - position),node);
      }
    }
  }
      ...
      switch (ch) {
        ...
        case '=':
          ...
          if (code.length() > position && !Character.isWhitespace(code.charAt(position))) {
            createStartIssue(node.getStartLinePosition(), node.getStartColumnPosition() + position,node);
          }
          break;
        default:
          if (!Character.isWhitespace(ch)) {
  ...
  private void createStartIssue(int line, int expectedWhitespaceColumn,Node node) {
    createViolation(line, "A whitespace is missing after the starting tag at column " + expectedWhitespaceColumn, node);
  }
