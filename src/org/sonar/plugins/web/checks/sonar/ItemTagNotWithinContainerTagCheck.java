...
public class ItemTagNotWithinContainerTagCheck extends AbstractPageCheck {
  ...
  public void startElement(TagNode node) {
    if (isLi(node) && !hasLiOrUlOrOlAncestor(node)) {
      createViolation(node.getStartLinePosition(), "Surround this <" + node.getNodeName() + "> item tag by a <ul> or <ol> container one.",node);
    } else if (isDt(node) && !hasDtOrDlAncestor(node)) {
      createViolation(node.getStartLinePosition(), "Surround this <" + node.getNodeName() + "> item tag by a <dl> container one.",node);
    }
  }
