    ...
    if (isInputRequiredLabel(node) || isSelect(node) || isTextarea(node)) {
      ...
      if (id == null) {
        createViolation(node.getStartLinePosition(),
        		"Add an \"id\" attribute to this input field and associate it with a label.",node);
      } else {
  ...
  public void endDocument() {
    for (Map.Entry<String, Integer> entry : inputIdToLine.entrySet()) {
      if (!labelFor.contains(entry.getKey())) {
        createViolation(entry.getValue(), "Associate a valid label to this input field.",null);
      }
    }
  }
